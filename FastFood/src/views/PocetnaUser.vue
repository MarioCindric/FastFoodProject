...
const trenutnaOcjena = ref(0)
const prosjecnaOcjena = ref(null)

async function otvoriDialog(jelo) {
  odabranoJelo.value = jelo
  prosjecnaOcjena.value = null
  trenutnaOcjena.value = 0
  dialogVidljiv.value = true

  try {
    const resProsjek = await axios.get(`/api/OcjenaJela/prosjecna/${jelo.id}`)
    prosjecnaOcjena.value = resProsjek.data
  } catch {
    prosjecnaOcjena.value = null
  }
  ...
  if (korisnikIdFinal) {
    try {
      const res = await axios.get(`/api/OcjenaJela/ocjene/${jelo.id}/${korisnikIdFinal}`)
      trenutnaOcjena.value = res.data[0]?.ocjena || 0
    } catch {
      trenutnaOcjena.value = 0
    }
  }
...
async function potvrdiOcjenu() {
  const user = JSON.parse(localStorage.getItem('user'))
  const korisnikIdFinal = user?.id || null
  if (!korisnikIdFinal || !odabranoJelo.value) return

  if (!trenutnaOcjena.value) {
    try {
      await axios.delete(`/api/OcjenaJela/delete/${odabranoJelo.value.id}/${korisnikIdFinal}`)
      dialogVidljiv.value = false
    } catch (err) {
      toast.add({
        severity: 'error',
        summary: 'Greška',
        detail: 'Brisanje ocjene nije uspjelo',
        life: 3000
      })
    }
    return
  ...
  const body = {
    jeloId: odabranoJelo.value.id,
    ocjena: trenutnaOcjena.value,
    korisnikId: korisnikIdFinal
  }

  try {
    await axios.post('/api/OcjenaJela/ocijeni', body)
    dialogVidljiv.value = false
  } catch (err) {toast.add({severity: 'error', summary: 'Greška', detail: 'Spremanje ocjene nije uspjelo', life: 3000
    })
  }
}
  ...
  <div v-for="(grupa, naziv) in grupiranaJela" :key="naziv" class="mb-5">
    ...
    <div class="row">
      <div v-for="jelo in grupa" :key="jelo.id" class="col-md-4 mb-4">
        <div class="card h-100 d-flex flex-row align-items-center position-relative" style="min-height: 200px;"@click="otvoriDialog(jelo)">
...
>
  ...
  >
    <h4 class="mb-0" style="margin-top: -1rem;">Ocijeni jelo</h4>
    ...
    <span class="text">Moja ocjena</span>
    <Rating v-model="trenutnaOcjena" :cancel="false" />
  </div>

  <template #footer>
    <div class="d-flex justify-content-end gap-2 w-100 px-3 pb-3">
      <Button label="Zatvori" severity="danger" @click="dialogVidljiv = false" />
      <Button label="Potvrdi" icon="pi pi-check" severity="success" @click="potvrdiOcjenu" />
    </div>
  </template>
</Dialog>
